...
class Match(object):
    ...
    def match_data(self, year, start_round, end_round, TM):
        match_dict = {
            '2021' : 6185,
            '2020' : 5795,
            '2019' : 5126,
                    ...
                    for key in response.json():
                        ...
                        if ohtml.xpath('//table[@id="datatb"]//tr[10]/td[@class="tb_plgs"]/@title'):
                            ...
                            if not os.path.exists(f'{year}-{start_round}-{end_round}.csv'):
                                ...
                                csvFile = open(f'{year}-{start_round}-{end_round}.csv', 'a', newline='', encoding='utf-8-sig')
                                ...
                                csvFile = open(f'{year}-{start_round}-{end_round}.csv', 'a+', newline='', encoding='utf-8-sig')
                                writer = csv.writer(csvFile)
                                writer.writerows([info, info1, info2, []])
                                csvFile.close()
                        ...
                        # 第10组赔率公司不存在时
                            ...
                            # 存储
                            info = [year, rou, ouzhi_url, '', tname, peilv, gailv, beidan, bifa, chengjiaojia,
                                    chengjiaoliang, zhuangjia, bifazhishu, lengre, yingkui, '', '']

                            info1 = ['', '', '', '', tname2, peilv2, gailv2, beidan2, bifa2, chengjiaojia2,
                                     chengjiaoliang2, zhuangjia2, bifazhishu2, lengre2, yingkui2,
                                     f'{tname}-{tname3} {bifen}',
                                     '', name, oupeisheng, oupeisheng2, float(oupeisheng) - float(oupeisheng2),
                                     oupeiping, oupeiping2, oupeifu, oupeifu2, float(oupeifu) - float(oupeifu2),
                                     gailvsheng, gailvsheng2, gailvshengcha, gailvping, gailvping2, gailvfu, gailvfu2,
                                     gailvfucha, fanhuanlv, fanhuanlv2, fanhuanlvcha, kailisheng, kailisheng2,
                                     float(kailisheng) - float(kailisheng2), kailiping, kailiping2, kailifu, kailifu2,
                                     float(kailifu) - float(kailifu2),
                                     '', name3, oupeisheng3, oupeisheng4, float(oupeisheng3) - float(oupeisheng4),
                                     oupeiping3, oupeiping4, oupeifu3, oupeifu4, float(oupeifu3) - float(oupeifu4),
                                     gailvsheng3, gailvsheng4, gailvshengcha4, gailvping3, gailvping4, gailvfu3,
                                     gailvfu4, gailvfucha4, fanhuanlv3, fanhuanlv4, fanhuanlvcha4, kailisheng3,
                                     kailisheng4, float(kailisheng3) - float(kailisheng4), kailiping3, kailiping4,
                                     kailifu3, kailifu4, float(kailifu3) - float(kailifu4),
                                     '', name5, oupeisheng5, oupeisheng6, float(oupeisheng5) - float(oupeisheng6),
                                     oupeiping5, oupeiping6, oupeifu5, oupeifu6, float(oupeifu5) - float(oupeifu6),
                                     gailvsheng5, gailvsheng6, gailvshengcha6, gailvping5, gailvping6, gailvfu5,
                                     gailvfu6, gailvfucha6, fanhuanlv5, fanhuanlv6, fanhuanlvcha6, kailisheng5,
                                     kailisheng6, float(kailisheng5) - float(kailisheng6), kailiping5, kailiping6,
                                     kailifu5, kailifu6, float(kailifu5) - float(kailifu6),
                                     '', name7, oupeisheng7, oupeisheng8, float(oupeisheng7) - float(oupeisheng8),
                                     oupeiping7, oupeiping8, oupeifu7, oupeifu8, float(oupeifu7) - float(oupeifu8),
                                     gailvsheng7, gailvsheng8, gailvshengcha8, gailvping7, gailvping8, gailvfu7,
                                     gailvfu8, gailvfucha8, fanhuanlv7, fanhuanlv8, fanhuanlvcha8, kailisheng7,
                                     kailisheng8, float(kailisheng7) - float(kailisheng8), kailiping7, kailiping8,
                                     kailifu7, kailifu8, float(kailifu7) - float(kailifu8),
                                     '', name9, oupeisheng9, oupeisheng10, float(oupeisheng9) - float(oupeisheng10),
                                     oupeiping9, oupeiping10, oupeifu9, oupeifu10, float(oupeifu9) - float(oupeifu10),
                                     gailvsheng9, gailvsheng10, gailvshengcha10, gailvping9, gailvping10, gailvfu9,
                                     gailvfu10, gailvfucha10, fanhuanlv9, fanhuanlv10, fanhuanlvcha10, kailisheng9,
                                     kailisheng10, float(kailisheng9) - float(kailisheng10), kailiping9, kailiping10,
                                     kailifu9, kailifu10, float(kailifu9) - float(kailifu10),
                                     '', name11, oupeisheng11, oupeisheng12, float(oupeisheng11) - float(oupeisheng12),
                                     oupeiping11, oupeiping12, oupeifu11, oupeifu12,
                                     float(oupeifu11) - float(oupeifu12), gailvsheng11, gailvsheng12, gailvshengcha12,
                                     gailvping11, gailvping12, gailvfu11, gailvfu12, gailvfucha12, fanhuanlv11,
                                     fanhuanlv12, fanhuanlvcha12, kailisheng11, kailisheng12,
                                     float(kailisheng11) - float(kailisheng12), kailiping11, kailiping12, kailifu11,
                                     kailifu12, float(kailifu11) - float(kailifu12),
                                     '', name13, oupeisheng13, oupeisheng14, float(oupeisheng13) - float(oupeisheng14),
                                     oupeiping13, oupeiping14, oupeifu13, oupeifu14,
                                     float(oupeifu13) - float(oupeifu14), gailvsheng13, gailvsheng14, gailvshengcha14,
                                     gailvping13, gailvping14, gailvfu13, gailvfu14, gailvfucha14, fanhuanlv13,
                                     fanhuanlv14, fanhuanlvcha14, kailisheng13, kailisheng14,
                                     float(kailisheng13) - float(kailisheng14), kailiping13, kailiping14, kailifu13,
                                     kailifu14, float(kailifu13) - float(kailifu14),
                                     '', name15, oupeisheng15, oupeisheng16, float(oupeisheng15) - float(oupeisheng16),
                                     oupeiping15, oupeiping16, oupeifu15, oupeifu16,
                                     float(oupeifu15) - float(oupeifu16), gailvsheng15, gailvsheng16, gailvshengcha16,
                                     gailvping15, gailvping16, gailvfu15, gailvfu16, gailvfucha16, fanhuanlv15,
                                     fanhuanlv16, fanhuanlvcha16, kailisheng15, kailisheng16,
                                     float(kailisheng15) - float(kailisheng16), kailiping15, kailiping16, kailifu15,
                                     kailifu16, float(kailifu15) - float(kailifu16),
                                     '', name17, oupeisheng17, oupeisheng18, float(oupeisheng17) - float(oupeisheng18),
                                     oupeiping17, oupeiping18, oupeifu17, oupeifu18,
                                     float(oupeifu17) - float(oupeifu18), gailvsheng17, gailvsheng18, gailvshengcha18,
                                     gailvping17, gailvping18, gailvfu17, gailvfu18, gailvfucha18, fanhuanlv17,
                                     fanhuanlv18, fanhuanlvcha18, kailisheng17, kailisheng18,
                                     float(kailisheng17) - float(kailisheng18), kailiping17, kailiping18, kailifu17,
                                     kailifu18, float(kailifu17) - float(kailifu18),
                                     ]

                            info2 = ['', '', '', '', tname3, peilv3, gailv3, beidan3, bifa3, chengjiaojia3,
                                     chengjiaoliang3, zhuangjia3, bifazhishu3, lengre3, yingkui3]

                            if not os.path.exists(f'{year}-{start_round}-{end_round}.csv'):
                                head = ['年份', '轮数', '百家欧赔url', '', '球队名字 (百家欧赔)', '赔率 (百家欧赔)', '概率 (百家欧赔)',
                                        '北单(-1) (交易比例)', '必发 (交易比例)', '成交价 (必发成交)', '成交量 (必发成交)', '庄家盈亏 (必发成交)',
                                        '必发指数 (指数分析)', '冷热指数 (指数分析)', '盈亏指数 (指数分析)', '对局方比分',
                                        '', '赔率公司定制 (定制一)', '胜 (及时欧赔)', '胜 (及时欧赔)', '差值', '平 (及时欧赔)', '平 (及时欧赔)',
                                        '负 (及时欧赔)', '负 (及时欧赔)', '差值', '胜 (及时概率)', '胜 (及时概率)', '差值', '平 (及时概率)',
                                        '平 (及时概率)', '负 (及时概率)', '负 (及时概率)', '差值', '值 (返还率)', '值 (返还率)', '返还率差',
                                        '胜 (凯利指数)', '胜 (凯利指数)', '差值', '平 (凯利指数)', '平 (凯利指数)', '负 (凯利指数)', '负 (凯利指数)',
                                        '差值',
                                        '', '赔率公司定制 (定制一)', '胜 (及时欧赔)', '胜 (及时欧赔)', '差值', '平 (及时欧赔)', '平 (及时欧赔)',
                                        '负 (及时欧赔)', '负 (及时欧赔)', '差值', '胜 (及时概率)', '胜 (及时概率)', '差值', '平 (及时概率)',
                                        '平 (及时概率)', '负 (及时概率)', '负 (及时概率)', '差值', '值 (返还率)', '值 (返还率)', '返还率差',
                                ...
                                csvFile = open(f'{year}-{start_round}-{end_round}.csv', 'a', newline='',
                                               encoding='utf-8-sig')
                                writer = csv.writer(csvFile)
                                writer.writerow(head)
                                csvFile.close()
